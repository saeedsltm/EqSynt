...
class Main():
    ...
    def createNewCatalogFile(self, computeWeight=True):
        ...
        print(
            "+++ Generating new catalog, wiehting={computeWeight} ...".format(computeWeight=computeWeight))
    ...
    def report2csv(self, reportFile):
        ...
        with open(reportFile) as f, open("{reportFile:s}.csv".format(reportFile=reportFile.split(".")[0]), "w") as g:
            ...
            for l in f:
                ...
                try:
                    ot = dt.strptime(l[:20], " %Y %m%d %H%M %S.%f").strftime(
                        "%Y-%m-%d %H:%M:%S.%f")
                except ValueError:
                    l = l[:16]+"59.99"+l[20:]
                    ot = dt.strptime(l[:20], " %Y %m%d %H%M %S.%f").strftime(
                        "%Y-%m-%d %H:%M:%S.%f")
                lat, latErr = self.handleMissingValue(
                    l[22:28]), self.handleMissingValue(l[29:34])
                lon, lonErr = self.handleMissingValue(
                    l[37:43]), self.handleMissingValue(l[44:49])
                dep, depErr = self.handleMissingValue(
                    l[50:55]), self.handleMissingValue(l[56:61])
