...
def computeClass(df_un, df_w, c, config):
    ...
    df_un = df_un[(df_un["ERH"]<=ERH)&(df_un["ERZ"]<=ERZ)&(df_un["GAP"]<=GAP)&(df_un["RMS"]<=RMS)&(df_un["MDS"]<=MDS)&(df_un["NuP"]>=NuP)&(df_un["NuS"]>=NuS)]
    df_w = df_w[(df_w["ERH"]<=ERH)&(df_w["ERZ"]<=ERZ)&(df_w["GAP"]<=GAP)&(df_w["RMS"]<=RMS)&(df_w["MDS"]<=MDS)&(df_w["NuP"]>=NuP)&(df_w["NuS"]>=NuS)]
...
def summarize(df_un, df_w, config, resultPath):
    ...
    with open(os.path.join(resultPath, "summary.dat"), "w") as f:
        ...
        for c in config["RPS"]["Classes"].keys():
            ...
            f.write("| Events relocated using weighting scheme |".center(40, "="))
            f.write("\n")
            NoEqInClass_un, PerEqInClass_un, NoEqInClass_w, PerEqInClass_w= computeClass(df_un, df_w, c, config)
            f.write("+++ Number(%) of event:".ljust(50, " "))
            f.write("{n:.0f}({p:.1f})".format(n=NoEqInClass_un, p=PerEqInClass_un).rjust(40, " " ))
            f.write("{n:.0f}({p:.1f})".format(n=NoEqInClass_w, p=PerEqInClass_w).rjust(40, " " ))
            f.write("\n")            