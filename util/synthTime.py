...
from util.extra import decoratorTimer
...
def generateVelocityGrid(velocityModelDict, nXnYnZ, node_intervals=(1., 1., 1.), min_coords=(0., 0., 0.), df=0):
    ...
    Vp, Z, VpVs = velocityModelDict["Vp"], velocityModelDict["Z"], velocityModelDict["VpVs"]
...
# @decoratorTimer(2)
def TravelTimeTable(solver, src_idx, savingPath):
    ...
    Args:
        ...
        savingPath (str): file for which the travel time table will be saved in
    """
    solver.traveltime.values[src_idx] = 0
    solver.unknown[src_idx] = False
    solver.trial.push(*src_idx)
    solver.solve()
    solver.traveltime.to_hdf(savingPath)


# @decoratorTimer(2)
def computeTTT(velocity, source_z, z_interval, savingPath):
    ...
    TravelTimeTable(solver, src_idx, savingPath)
...
def generateTTT(velocityModelDict, velocityType, numXGrid, numZGrid, node_intervals=(1., 1., 1.), decimationFactor=0):
    ...
    for source_z in tzip(range(numZGrid)):
        source_z = source_z[0]
        savingPath = os.path.join("ttt", "dep{z:003d}{vt:s}.hdf5".format(
            z=int(source_z), vt=velocityType))
        if os.path.exists(savingPath):
            os.remove(savingPath)
        if velocityType == "P":
            computeTTT(velocity_P, source_z, z_interval, savingPath)
        else:
            computeTTT(velocity_S, source_z, z_interval, savingPath)
